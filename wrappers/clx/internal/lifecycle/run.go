...
func Run(ctx context.Context, opts Options) (int, error) {
	...
	if err != nil {
		if errors.Is(err, ipc.ErrHeld) {
			fmt.Fprintln(os.Stderr, "clx: another instance is already running")
			return 1, err
		}
		return 1, err
	}
	...
	if !opts.SkipAuthSync {
		if err := syncAuth(ctx, client, logger); err != nil {
			logger.Warn("auth sync failed; continuing", "err", err)
		}
	}
...
func syncResources(ctx context.Context, client *orchestrator.Client, logger *slog.Logger) {
	...
	go func() {
		defer wg.Done()
		if err := writeSettings(syncCtx, client); err != nil {
...
func writeSettings(ctx context.Context, client *orchestrator.Client) error {
	home, _ := os.UserHomeDir()
