...
func TestPrintWrapperHelpRichUsesOrangeAndStaysWithinWidth(t *testing.T) {
	t.Parallel()
	...
	if !strings.Contains(got, caps.Palette.Orange) {
		t.Fatalf("rich help does not use the cdx orange accent:\n%q", got)
	}
	plain := StripANSI(got)
	for _, want := range []string{"CDX", "WRAPPER HELP", "cdx lane", "--wrapper-help"} {
