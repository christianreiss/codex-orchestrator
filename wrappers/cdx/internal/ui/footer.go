...
type ExitFooter struct {
	...
	AuthTone    Tone
	// CodexVersion is the post-install codex version when the auto-update
	// path swapped the binary on this run. Empty for the common no-op case;
	// when set, an extra `● codex X.Y.Z` badge is added to the Sync row.
	CodexVersion string
}
...
func PrintExitFooter(w io.Writer, caps Caps, prefix string, f ExitFooter) {
	...
	if f.CodexVersion != "" {
		syncLine += "  " + footerDot(caps, "codex "+f.CodexVersion, ToneOK)
	}
