...
// Frames are assembled in memory and emitted with one write, preventing a
// partial redraw from interleaving with a background status message.
func newFrame(w io.Writer, caps Caps) card {
	buf := &strings.Builder{}
	c := newCard(buf, caps)
	buf.Grow(c.width * 32)
	c.target = w
...
func (c card) bottom() {
	c.edge(c.caps.BannerSym.BoxBL, c.caps.BannerSym.BoxBR)
	if c.target != nil {
		_, _ = io.WriteString(c.target, c.w.(*strings.Builder).String())
	}
}
...
func (c card) edge(left, right string) {
	...
	accentWidth := 3
	if c.width-2 < accentWidth {
		accentWidth = c.width - 2
	}
	fmt.Fprintln(c.w, c.caps.BannerColor()+left+strings.Repeat(c.caps.BannerSym.BoxH, accentWidth)+r+p+strings.Repeat(c.caps.BannerSym.BoxH, c.width-2-accentWidth)+right+r)
}

func (c card) divider(label string) {
	c.line("")
	if strings.TrimSpace(label) != "" {
		label = TruncateText(strings.ToUpper(CleanInline(label)), c.inner, c.caps)
		c.line(c.caps.Palette.Dim + label + c.caps.Palette.Reset)
	}
}
...
func CleanInline(s string) string {
	if printableASCII(s) && !strings.HasPrefix(s, " ") && !strings.HasSuffix(s, " ") && !strings.Contains(s, "  ") {
		return s
	}
	s = StripANSI(s)
