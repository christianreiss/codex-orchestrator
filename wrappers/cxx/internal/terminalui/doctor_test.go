...
func TestPrintDoctorRichCardIsResponsiveAndSanitized(t *testing.T) {
	...
	for _, want := range []string{"CDX", "DOCTOR", "CHECKS", "GUIDANCE", "1 check failed", "✓", "!", "×"} {
	...
	if strings.Index(plain, "1 check failed") > strings.Index(plain, "CHECKS") {
		t.Fatal("actionable verdict must precede diagnostic details")
	}
