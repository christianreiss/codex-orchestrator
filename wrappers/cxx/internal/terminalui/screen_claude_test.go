...
func TestPrintBootScreenRichIsAtAGlanceAndResponsiveClaude(t *testing.T) {
	caps := screenCapsClaude(72)
	var buf bytes.Buffer
	printBootScreen(&buf, ScreenInput{Prefix: "clx", EngineName: "claude",
		WrapperVersion: "0.6.44", WrapperTarget: "0.6.45", WrapperTone: ToneWarn,
		EngineVersion: "2.1.206", EngineTone: ToneOK,
	...
	assertScreenLinesFitClaude(t, out, caps.Columns)
}

func TestPrintBootScreenSanitizesDynamicValuesClaude(t *testing.T) {
	caps := screenCapsClaude(48)
	var buf bytes.Buffer
	printBootScreen(&buf, ScreenInput{Prefix: "clx", EngineName: "claude",
		HostFQDN:       "node\x1b]2;FORGED\a.example\nsecond-row",
		EngineVersion:  "2.1.206\x1b[2J",
...
func TestPrintMinimalScreenIsStableAndLogSafeClaude(t *testing.T) {
	var buf bytes.Buffer
	PrintBootScreen(&buf, ScreenInput{Prefix: "clx", EngineName: "claude",
		WrapperVersion: "0.6.44", EngineVersion: "2.1.206", HostFQDN: "host.example",
...
func TestScreenKeepsEffortVisibleWithoutModelClaude(t *testing.T) {
	var rich bytes.Buffer
	printBootScreen(&rich, ScreenInput{Prefix: "clx", EngineName: "claude", Effort: "high"}, screenCapsClaude(64))
	...
	PrintMinimalScreen(&minimal, ScreenInput{Prefix: "clx", EngineName: "claude", Effort: "high"})
	if !strings.Contains(minimal.String(), "effort=high") {
		t.Fatalf("minimal screen hid effort-only context:\n%s", minimal.String())
	}
}

func TestCompactScreenAndFooterRespectDetectedWidthsClaude(t *testing.T) {
	for _, width := range []int{39, 80} {
		t.Run(fmt.Sprintf("width_%d", width), func(t *testing.T) {
			caps := screenCapsClaude(width)
			caps.IsTTY = false
			caps.Palette = Palette{Bold: "\x1b[1m", Reset: "\x1b[0m", Red: "\x1b[31m"}
			input := ScreenInput{Prefix: "clx", EngineName: "claude",
				HostFQDN:      strings.Repeat("very-long-host-", 8) + "é.example",
				EngineVersion: strings.Repeat("2.1.206-long-", 5), WrapperVersion: "0.6.44",
			...
			assertScreenLinesFitClaude(t, screen.String(), width)
			assertASCIIOutputClaude(t, screen.String())
			...
			assertScreenLinesFitClaude(t, footer.String(), width)
			assertASCIIOutputClaude(t, footer.String())
...
func TestRichConcurrentScreenKeepsHealthAtAGlanceClaude(t *testing.T) {
	caps := screenCapsClaude(64)
	var buf bytes.Buffer
	printBootScreen(&buf, ScreenInput{Prefix: "clx", EngineName: "claude",
...
func TestRichConcurrentScreenIsNeutralWhenNoActionIsRequiredClaude(t *testing.T) {
	caps := screenCapsClaude(64)
	var buf bytes.Buffer
	printBootScreen(&buf, ScreenInput{Prefix: "clx", EngineName: "claude", Concurrent: true}, caps)
	if strings.Contains(buf.String(), "\x1b[33m\x1b[1m") {
		t.Fatalf("normal concurrent state used warning styling:\n%s", buf.String())
	}
}

func TestConcurrentScreenKeepsDistinctResultFooterClaude(t *testing.T) {
	caps := screenCapsClaude(64)
	var buf bytes.Buffer
	printBootScreen(&buf, ScreenInput{Prefix: "clx", EngineName: "claude",
		Concurrent:  true,
...
func TestResultLabelIsBoundedWithoutHidingNormalErrorsClaude(t *testing.T) {
	caps := screenCapsClaude(40)
...
func TestBypassPermissionsUsesWarningSemanticsClaude(t *testing.T) {
	caps := screenCapsClaude(64)
	var buf bytes.Buffer
	printBootScreen(&buf, ScreenInput{Prefix: "clx", EngineName: "claude", BypassPermissions: true, ResultLabel: "Ready with warning."}, caps)
	plain := StripANSI(buf.String())
...
func TestPrintMinimalScreenShowsTargetsAndUsesPortableASCIIClaude(t *testing.T) {
	var buf bytes.Buffer
	PrintMinimalScreen(&buf, ScreenInput{Prefix: "clx", EngineName: "claude",
		EngineVersion: "2.1.206", EngineTarget: "2.1.207",
...
func TestExitFooterReportsMeasuredFailureClaude(t *testing.T) {
	caps := screenCapsClaude(64)
	var rich bytes.Buffer
	...
	assertScreenLinesFitClaude(t, rich.String(), caps.Columns)
...
func TestExitFooterEscalatesAuthFailureWithSuccessfulProcessClaude(t *testing.T) {
	caps := screenCapsClaude(64)
	var buf bytes.Buffer
...
func TestTerminalSanitizerRemovesCSIAndOSCClaude(t *testing.T) {
...
func TestDetectCapsForGenericWriterIsRedirectedClaude(t *testing.T) {
	var buf bytes.Buffer
...
func TestHealthAndQuotaPrimitivesClaude(t *testing.T) {
	caps := screenCapsClaude(80)
	var buf bytes.Buffer
...
func screenCapsClaude(columns int) Caps {
	return Caps{
...
func assertScreenLinesFitClaude(t *testing.T, output string, columns int) {
	t.Helper()
...
func assertASCIIOutputClaude(t *testing.T, output string) {
	t.Helper()
