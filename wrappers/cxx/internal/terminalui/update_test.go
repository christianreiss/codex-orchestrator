...
func TestUpdateLinesAreUniformAndColoured(t *testing.T) {
	...
	cases := []struct {
		...
		{"progress", UpdateProgress(caps, "cdx", "wrapper", "0.6.41", "0.6.42"), "↻ · cdx · wrapper · 0.6.41 → 0.6.42 · updating…", "\033[38;5;208m"},
		{"complete", UpdateComplete(caps, "cdx", "wrapper", "0.6.42", true), "✓ · cdx · wrapper · 0.6.42 · updated, restarting…", "\033[32m"},
		{"install", UpdateProgress(caps, "cdx", "clx", "", ""), "↻ · cdx · clx · installing…", "\033[38;5;208m"},
		{"failure", UpdateFailure(caps, "cdx", "wrapper", "0.6.42", errors.New("checksum mismatch")), "✗ · cdx · wrapper · 0.6.42 · update skipped: checksum mismatch", "\033[31m"},
	}
...
func TestUpdateLinesUseAsciiInDumbTerminals(t *testing.T) {
	caps := Caps{Dumb: true}
	if got, want := UpdateProgress(caps, "cdx", "wrapper", "0.6.41", "0.6.42"), "~ | cdx | wrapper | 0.6.41 -> 0.6.42 | updating..."; got != want {
...
func TestUpdateLinesUseBoundedAsciiWhenRedirected(t *testing.T) {
	...
	line := UpdateFailure(caps, "cdx", "wrapper", "0.6.44", errors.New(strings.Repeat("broken ", 20)))
...
func TestNarrowUpdateLinesPreserveOutcomeBeforeMetadata(t *testing.T) {
	...
	cases := []struct {
		...
		{"progress", UpdateProgress(caps, "cdx", "wrapper", "0.6.44-build-metadata", "0.6.45-build-metadata"), "updating..."},
		{"complete", UpdateComplete(caps, "cdx", "wrapper", "0.6.45-build-metadata", true), "updated, restarting..."},
		{"failure", UpdateFailure(caps, "cdx", "wrapper", "0.6.45-build-metadata", errors.New("checksum mismatch with detailed context")), "update skipped"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
