...
func TestExitFooterVersionDoesNotImplyAnUpdate(t *testing.T) {
	for _, version := range []string{"1.2.3", "unknown"} {
		var out bytes.Buffer
		PrintExitFooter(&out, footerCaps(80), "cdx", ExitFooter{EngineName: "codex", EngineVersion: version})
		plain := StripANSI(out.String())
		if !strings.Contains(plain, "codex "+version) || strings.ContainsAny(plain, "↑^") {
			t.Fatalf("unmeasured version metadata implies an update: %q", plain)
		}
	}
}
