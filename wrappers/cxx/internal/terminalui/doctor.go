...
func PrintDoctor(w io.Writer, caps Caps, report DoctorReport) {
	caps = engineCaps(caps, report.Engine)
...
func printDoctorCard(w io.Writer, caps Caps, report DoctorReport) {
	...
	counts := []string{fmt.Sprintf("%d passed", passed)}
	if warnings > 0 {
		label := "warnings"
		if warnings == 1 {
			label = "warning"
		}
		counts = append(counts, fmt.Sprintf("%d %s", warnings, label))
	}
	if failed > 0 {
		counts = append(counts, fmt.Sprintf("%d failed", failed))
	}
	c.line(caps.Palette.Dim + strings.Join(counts, richSeparator(caps)) + caps.Palette.Reset)
	if report.Result.Value != "" {
		renderToneText(c, report.Result.Tone, report.Result.Value)
	}
	c.divider("Checks")
	...
	if len(report.Hints) > 0 || warnings > 0 || failed > 0 {
		c.divider("Guidance")
