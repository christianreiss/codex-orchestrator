...
func TestCompactNumber(t *testing.T) {
	cases := []struct {
		...
		{10_000_000, "10M"},
		{-1_500, "-1.5K"},
		{math.MinInt64, "-9223372036854M"},
		{math.MaxInt64, "9223372036854M"},
	}
	for _, c := range cases {
...
func TestFormatQuotaLinesHonorProjectionTone(t *testing.T) {
	caps := Caps{
	...
	for _, tc := range []struct {
		...
		line := strings.Join(formatQuotaLines(caps, QuotaRow{Label: "5h", Used: 20, Projection: "~47% at reset", ProjectionTone: tc.tone}, 80), "\n")
