...
func PrintExitFooter(w io.Writer, caps Caps, prefix string, f ExitFooter) {
	...
	c := newFrame(w, caps)
	brand := caps.BannerColor() + prefix + caps.Palette.Reset + caps.Palette.Dim + "  /  " + caps.Palette.Reset + caps.Palette.Bold + "SESSION" + caps.Palette.Reset
	c.top()
	...
	if engine != "" && version != "" {
		pieces = append(pieces, caps.Palette.Dim+engine+caps.Palette.Reset+" "+version)
	}
