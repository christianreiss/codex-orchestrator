...
func UpdateProgress(caps Caps, engine, component, current, target string) string {
	...
	return formatUpdate(caps, caps.BannerColor(), updateGlyph(caps, "↻", "~"), engine, component, updateVersion(caps, current, target), status)
}
...
func updateCaps(caps Caps, engine string) Caps {
	caps = engineCaps(caps, engine)
	if caps.Columns > 0 && caps.Columns < minRichColumns {
