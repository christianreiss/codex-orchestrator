...
func TestFetchBundleForbiddenReturnsSentinel(t *testing.T) {
	...
	_, err := fleetconfig.Fetch(context.Background(), cfg, peerEngine)
...
func TestPeerBinaryCurrentScansShadowedPath(t *testing.T) {
	...
	path, ok := matchingCommonBinary(hex.EncodeToString(sum[:]))
	if !ok {
		t.Fatal("expected peerBinaryCurrent to find current peer behind stale PATH shadow")
	}
	if path != filepath.Join(currentDir, peerName) {
		t.Fatalf("matching path=%q", path)
	}
}
...
func TestCoordinatedCronChildNeverSpawnsPeer(t *testing.T) {
	t.Setenv(coordinatedCronEnv, "1")
	EnsureForCron(context.Background(), nil, true, slog.New(slog.NewTextHandler(io.Discard, nil)))
}

func TestClaudeDisablesCodexWithoutRemovingSharedCronOrExecutingPeer(t *testing.T) {
	dir := t.TempDir()
	home := filepath.Join(dir, "home")
	if err := os.MkdirAll(home, 0o755); err != nil {
		t.Fatal(err)
	}
	marker := filepath.Join(dir, "peer-was-executed")
	peer := filepath.Join(dir, peerName)
	if err := os.WriteFile(peer, []byte("#!/bin/sh\ntouch '"+marker+"'\n"), 0o755); err != nil {
		t.Fatal(err)
	}
	sharedCron := filepath.Join(dir, "cxx-managed")
	if err := os.WriteFile(sharedCron, []byte("shared"), 0o644); err != nil {
		t.Fatal(err)
	}
	oldLegacyCron, oldLegacyOpt := legacyPeerCronPath, legacyPeerOptPath
	legacyPeerCronPath = filepath.Join(dir, "cdx-managed")
	legacyPeerOptPath = filepath.Join(dir, "opt-codex")
	t.Cleanup(func() { legacyPeerCronPath, legacyPeerOptPath = oldLegacyCron, oldLegacyOpt })
	t.Setenv("HOME", home)
	t.Setenv("PATH", dir)
	t.Setenv("CDX_CONFIG_PATH", filepath.Join(dir, "cdx.json"))
	if err := removePeer(context.Background(), slog.New(slog.NewTextHandler(io.Discard, nil))); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(marker); !os.IsNotExist(err) {
		t.Fatalf("peer cron remove was executed: %v", err)
	}
	if got, err := os.ReadFile(sharedCron); err != nil || string(got) != "shared" {
		t.Fatalf("shared cron changed: %q err=%v", got, err)
	}
}
