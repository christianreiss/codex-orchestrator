...
var legacyPeerCronPath = "/etc/cron.d/cdx-managed"
var legacyPeerOptPath = "/opt/codex"
...
func installPeer(ctx context.Context, cfg *config.Config, forceCronTick, minimal bool, logger *slog.Logger) error {
	...
	if !current {
		...
		fmt.Fprintln(os.Stderr, ui.UpdateProgress(caps, "clx", peerName, "", ""))
		source, err = coreupdate.Install(ctx, peerCfg, peerCfg.Wrapper.BinaryURL, sum, peerCfg.Wrapper.Version, slog.Default())
		if err != nil {
			fmt.Fprintln(os.Stderr, ui.UpdateFailure(caps, "clx", peerName, "", err))
			return err
		}
		fmt.Fprintln(os.Stderr, ui.UpdateComplete(caps, "clx", peerName, "", false))
		installed = true
	}
...
func peerConfigPath() string {
	...
	if err != nil || home == "" {
		return filepath.Join(os.TempDir(), "codex-orchestrator-no-home", "cdx.json")
	}
...
func removePeer(ctx context.Context, logger *slog.Logger) error {
	...
	if npmGlobalHas("codex-cli") {
		if out, err := exec.CommandContext(ctx, "npm", "uninstall", "-g", "codex-cli").CombinedOutput(); err != nil {
			logger.Warn("peer npm uninstall skipped", "package", "codex-cli", "err", err, "output", strings.TrimSpace(string(out)))
		}
	}
	removeTreePath(legacyPeerOptPath, logger)
