...
func TestCodexDisablesClaudeWithoutRemovingSharedCronOrExecutingPeer(t *testing.T) {
	dir := t.TempDir()
	...
	oldLegacy := legacyPeerCronPath
	legacyPeerCronPath = filepath.Join(dir, "clx-managed")
	t.Cleanup(func() { legacyPeerCronPath = oldLegacy })
	t.Setenv("HOME", home)
	t.Setenv("PATH", dir)
	t.Setenv("CLX_CONFIG_PATH", filepath.Join(dir, "clx.json"))
