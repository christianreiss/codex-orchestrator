...
" "$settings_path" 2>&1)" || config_parse_err="parse error"
  ...
  if command -v crontab >/dev/null 2>&1; then
    local clx_real_path=""
    clx_real_path="$(realpath "$0" 2>/dev/null || readlink -f "$0" 2>/dev/null || echo "$0")"
    local cron_marker=""
    cron_marker="$(cron_managed_marker)"
    local current_crontab=""
    current_crontab="$(crontab -l 2>/dev/null || true)"
    local quoted_clx_path=""
    printf -v quoted_clx_path '%q' "$clx_real_path"
    if cron_has_wrapper_entry "$current_crontab" "$clx_real_path" "$quoted_clx_path" "$cron_marker"; then
