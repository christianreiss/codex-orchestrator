...
def rewrite_output_buffer(buffer, final):
    scan_end = len(buffer) if final else max(0, len(buffer) - tail_len)
    chunk = buffer[:scan_end]
    remainder = b"" if final else buffer[scan_end:]
    cpr_count = len(cpr_re.findall(chunk))
    if cpr_count:
        chunk = cpr_re.sub(b"", chunk)
    chunk = output_filter_re.sub(b"", chunk)
    return chunk, remainder, cpr_count
        ...
        while True:
            ...
            if child_fd in ready:
                ...
                if child_data:
                    ...
                    emit, pending_output, cpr_count = rewrite_output_buffer(pending_output, final=False)
                    if cpr_count:
                        os.write(child_fd, cpr_reply * cpr_count)
                    write_stdout_and_log(handle, emit)
                else:
                    emit, pending_output, cpr_count = rewrite_output_buffer(pending_output, final=True)
                    if cpr_count:
                        os.write(child_fd, cpr_reply * cpr_count)
                    write_stdout_and_log(handle, emit)
                    break
...
run_codex_command() {
  ...
  if [[ -t 0 && -t 1 ]]; then
    if ((CODEX_SSH_PTY_BRIDGE_ACTIVE)); then
      run_codex_command_via_python_pty_bridge "$tmp_output" "${exec_cmd[@]}"
      status=$?
    else
      "${exec_cmd[@]}"
      status=$?
