...
- **Every admin mutation was open to every signed-in account; authorization is
  ...
  a role name is compared to anything — and
  `api/src/security/route-capabilities.ts` assigns exactly one capability to
  every route under `/admin/*` and to the session-guarded `/cli/auth/*` routes.
  `api/src/http/plugins/capabilities.ts` attaches the guard as each route
  registers and **refuses to start** when a governed route has no entry, so a
  new endpoint cannot ship session-only by omission. Denials answer `403`
  `admin_role_required` with a `required_capability` field, before the handler
  runs. The six old gates are gone; `api/src/routes/` now contains no role
  comparison at all, and a test fails if one grows back.
  - Deliberately *not* widened: the 33 routes those gates covered are pinned in
    `api/test/unit/security/capability-layer-invariants.test.ts` and must still
    resolve to a capability no role beyond `owner` and `admin` holds. Host
    delete, registration, engine change, secure-state transition and CLI
    approval got their own `hosts.security_transition` for exactly this reason —
    folding them into `hosts.manage` would have handed them to `fleet_operator`.
  - `secrets.reveal`, `agent_portal.reveal_link`,
    `agent_messaging.reveal_content` and `auth.reveal_credential` are separate
    from their domains' `.read`: each returns bearer material or private
    content, and a listing beside them stays metadata-only.
  - **Closes a live credential leak.** `GET /admin/hosts/{id}/auth?include_body=1`
    returns the canonical credential the fleet distributes to hosts, and was
    reachable by any signed-in account including a `viewer`. It now needs
    `auth.reveal_credential`, held by `owner` and `admin` only — deliberately
    not by `fleet_operator`, which holds `auth.manage` and may replace the
    fleet's credential but not read the live one back out. Without the flag the
  ...
  - `GET /admin/auth/status` now returns the caller's `capabilities`. The admin
    ...
    hand-rolled `owner|admin` derivations in the UI. Presentation only — the
    server re-checks every request. A parity test fails when the console's copy
    of the vocabulary drifts from the API's.
  - `docs/ADMIN.md` carried the sentence "There is no capability system in the
    Node API"; the matrix table there is now generated by
    `npm run docs:capabilities` and held against `ROLE_CAPABILITIES` by a test.
...
- Finished the compact visual-system pass across project detail, Activity tabs,
  the Memory Inspector, standalone CLI approval, and shared controls/overlays.
...
- Reworked account security into one compact, route-backed workspace: password
  changes and recovery are sequential sections, registered passkeys use the
  available table width with a sticky header, and the account links are a
  concise horizontal navigation strip instead of a secondary desktop column.
- Recast the Claude client editor as one full-width operational sequence:
  fleet revision/save state, advisor, environment, permissions, status line,
  hooks, and the effective settings preview now follow the task order without
  nested cards or a competing fixed side column. Long serialized previews are
  keyboard-focusable for reliable review in every editor that uses them.
- Rebuilt the Engines workspace into one ordered, full-width configuration
  sequence. Codex, Claude, quota, and scaling controls no longer compete in
  desktop-only columns; scaling tiers use compact divided rows and now surface
  the live effective tier, lane, effort, and model alongside the editable rules.
- Fixed the dashboard release alert to consume the current structured release
  response while retaining compatibility with the earlier string response.
...
- Added derived `GET /admin/setup/status` and `/readyz` readiness, a serialized one-time first-owner claim that immediately signs in, a blocking `/admin/setup` UI, and a persistent engine-aware provider/host/sync checklist on the dashboard. `/healthz` remains liveness-only and setup-status becomes session-gated after the first owner exists.
...
- Added **Memory Atlas**, a graph-first admin workspace for the three deliberate
  ...
  graph filters remain valid when aggregating tag facets across all scopes, and
  the canvas reserves an explicit viewport so nodes and controls render instead
  of collapsing inside an auto-height page section.
