# 2026-04-10
- Setup: `bin/setup.sh` seeds `PUBLIC_BASE_URL` automatically from the chosen Codex host URL instead of leaving it unset beside `CODEX_SYNC_BASE_URL` / `AUTH_RUNNER_CODEX_BASE_URL`. Existing explicit public-base overrides are preserved, but fresh/default installs now bake installer and wrapper URLs correctly without a second manual env edit.
