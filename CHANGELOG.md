# 2026-03-24
- cdx wrapper: interactive SSH launches now default to Codex inline mode (`--no-alt-screen`) when handing the terminal directly to `codex`, because some SSH terminals/hosts were regressing into immediate fullscreen UI failure with alt-screen enabled. `CODEX_FORCE_PTY=1` still forces wrapper PTY capture, and `CODEX_SSH_ALT_SCREEN=0` keeps fullscreen mode if an operator explicitly wants it. Wrapper bumped to `2026.03.24-06`.
