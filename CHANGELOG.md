...
- Wrapper v2 installer: when a host has stale global `cdx`/`clx` binaries in `/usr/local/bin` or `/usr/local/sbin`, the installer now mirrors the freshly downloaded wrapper over those locations when writable or passwordless sudo is available, and prints the exact `sudo install` remediation otherwise.
...
- Admin WebUI: restored eight features that did not make the SvelteKit rewrite, in a single coordinated landing built across four parallel git worktrees.
  ...
  - Dashboard — new **Runner state** card displays current state, last-run timestamp, last error, last result, with **Run Codex runner** and **Verify Claude** buttons. Polls `/admin/runner` every 15 s; mutations invalidate on completion. Note: the Node API's runner endpoints (`/admin/runner`, `/admin/runner/run`, `/admin/runner/run-claude`) are still stubbed at the proxy layer in this build; the card degrades gracefully and surfaces the underlying 503 as a toast until the runner client lands.
...
- OpenAI-compatible API: added a minimal `POST /v1/responses` shim for non-streaming official SDK clients, and fixed `POST /v1/chat/completions` streaming to emit SDK-compatible `chat.completion.chunk` SSE payloads with `choices[].delta.content` instead of a single full completion object. Admin API reference copy and interface docs now describe the new endpoint and current streaming limits.
