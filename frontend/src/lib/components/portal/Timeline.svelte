<script lang="ts">
  ...
  import { buildTimeline, visibleTimeline } from "$lib/portal/grouping";
  ...
  >
    {#if items.length === 0}
      <div class="grid h-full place-content-center text-center text-muted-foreground">
        <MessageSquareDashedIcon class="mx-auto hidden h-8 w-8 opacity-60 sm:block" />
        <p class="text-body-sm sm:mt-2">No visible messages yet.</p>
      </div>
    {/if}
    ...
    {#each items as item (item.id)}
      ...
      {:else if item.role === "prompt"}
        <PromptCard event={item.event} active={false} onanswer={() => {}} readonly />
