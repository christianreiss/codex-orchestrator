...
const KEYS = {
  ...
  remoteExec: ["settings", "remote-exec"],
...
const QUERY_CASES: QueryCase[] = [
  ...
  {
    name: "remoteExecQuery",
    build: () => asQuery(settings.remoteExecQuery()),
    queryKey: KEYS.remoteExec,
    method: "GET",
    path: "/admin/remote-exec",
  },
  {
...
const MUTATION_CASES: MutationCase[] = [
  ...
  {
    name: "remoteExecMutation",
    build: (opts) => asMutation(settings.remoteExecMutation(opts)),
    variables: true,
    request: { method: "POST", path: "/admin/remote-exec", body: { enabled: true } },
    invalidates: [KEYS.remoteExec],
  },
  {
