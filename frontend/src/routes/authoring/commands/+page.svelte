<script lang="ts">
  ...
  const createMut = createMutation({
    mutationFn: (payload: { slug: string; description: string }) =>
      commandsApi.store({ slug: payload.slug, description: payload.description, body: "" }),
    onSuccess: (data, variables) => {
      toast.success(`Command "${variables.slug}" created`);
      void qc.invalidateQueries({ queryKey: commandsKeys.all });
      createOpen = false;
      newSlug = "";
      newDescription = "";
      void goto(`${base}/authoring/commands/${encodeURIComponent(data.slug ?? variables.slug)}`);
    },
    onError: (err: unknown) => {
  ...
  const deleteMut = createMutation({
    mutationFn: (slug: string) => commandsApi.delete(slug),
...
<div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
  ...
  <div class="flex flex-wrap items-center justify-between gap-2 sm:justify-end">
    ...
    <Button
      ...
      size="icon"
      aria-label="Refresh"
      onclick={() => void qc.invalidateQueries({ queryKey: commandsKeys.all })}
      ...
      {:else if $query.isError}
        <Table.Row>
          <Table.Cell colspan={5} class="py-6 text-center text-sm text-destructive">
            {$query.error instanceof Error ? $query.error.message : "Failed to load commands"}
          </Table.Cell>
        </Table.Row>
        ...
        <Table.Row>
          <Table.Cell colspan={5}>
            <EmptyState
              ...
              description="Commands are reusable slash-command prompts Claude can run on demand."
            >
              {#snippet action()}
                <Button size="sm" onclick={() => (createOpen = true)}>
                  <Plus class="h-4 w-4" />
        ...
        <Table.Row>
          <Table.Cell colspan={5}>
            <EmptyState
              icon={Search}
              size="sm"
              title={`No commands match "${search.trim()}"`}
        ...
        {#each sorted as row (row.slug)}
          ...
          <Table.Row>
            <Table.Cell class="font-medium">
              <a
                href={`${base}/authoring/commands/${encodeURIComponent(row.slug)}`}
            ...
            <Table.Cell class="text-right">
              <div class="inline-flex items-center gap-1">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  aria-label={`Delete command ${row.display_name || row.slug}`}
...
<Sheet.Root bind:open={createOpen}>
  <Sheet.Content side="right" class="w-full sm:max-w-md">
    <Sheet.Header>
      <Sheet.Title>New command</Sheet.Title>
      <Sheet.Description>
        Create an empty command. You'll be redirected to the editor on save.
      </Sheet.Description>
    </Sheet.Header>
    <div class="mt-6 space-y-4">
      <div class="space-y-1.5">
        <label for="new-command-slug" class="text-sm font-medium">Slug</label>
        <Input
          id="new-command-slug"
      ...
      <div class="space-y-1.5">
        <label for="new-command-description" class="text-sm font-medium">Description</label>
        <Textarea
    ...
    <Sheet.Footer class="mt-6 flex justify-end gap-2">
      ...
      <Button onclick={handleCreate} disabled={$createMut.isPending}>
        {$createMut.isPending ? "Creating…" : "Create command"}
      </Button>
    </Sheet.Footer>
  </Sheet.Content>
</Sheet.Root>
