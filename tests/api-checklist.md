...
1) Host provisioning
- Create host + installer → `200 ok` with host payload (including `api_key`) and `installer`: `curl -s -X POST http://localhost:8488/admin/hosts/register -H 'Content-Type: application/json' -H 'X-mTLS-Present: 1' -H "Cookie: <admin-session>" -d '{"fqdn":"host.test"}'`
...
- Missing canonical (fresh DB) → `status=missing` + `action=store`: `curl -s -X POST http://localhost:8488/auth -H "X-API-Key: $API_KEY" -H 'Content-Type: application/json' -d '{"command":"retrieve","digest":"aa...","last_refresh":"2025-11-23T00:00:00Z","client_version":"0.0.0"}'`
...
- Set flag (requires admin session cookie): `curl -s -X POST http://localhost:8488/admin/api/state -H 'Content-Type: application/json' -H 'X-mTLS-Present: 1' -H "Cookie: <admin-session>" -d '{"disabled":true}'`
...
- Enable roaming: `curl -s -X POST http://localhost:8488/admin/hosts/1/roaming -H 'X-mTLS-Present: 1' -H "Cookie: <admin-session>" -d '{"allow":true}'` then retry `/auth` with new IP → succeeds and updates IP (requires valid admin session; `X-mTLS-Present` alone is not sufficient).
...
9) Admin visibility
- Overview (requires admin session cookie): `curl -s -H 'X-mTLS-Present: 1' -H "Cookie: <admin-session>" http://localhost:8488/admin/overview`
- Logs (limit 5, requires admin session cookie): `curl -s -H 'X-mTLS-Present: 1' -H "Cookie: <admin-session>" 'http://localhost:8488/admin/logs?limit=5'`
