...
- `POST /admin/projects` — create one. Body: `{ slug, about?, roster_markdown?, board_template? }`. `agents_markdown` is accepted as a legacy alias for `roster_markdown`; both map to the same field. `slug` must be a URL-safe short identifier.
...
**Lanes and roles.** The migration seeds seven columns, duplicated as `SEEDED_COLUMNS` in `project-board.ts` and pinned against the SQL by a test: `backlog` (intake) → `planning` (`plan`) → `coding` (`code`) → `review` (`review`) → `verifying` (`verify`) → `done` (terminal), plus `blocked`. That set is the **software** board template, and it is the default. `POST /admin/projects` and `project_create` accept `board_template` to pick another: **migration** gives `backlog` → `discovery` → `plan` → `cutover` → `verify` → `done`, plus `blocked`, which is the shape infrastructure work has. The choice is made at creation and provisioned then, because the board is otherwise built lazily on first touch; every template is checked for one intake, one terminal and one blocked lane, a chain that reaches the terminal lane, and roles the fleet vocabulary declares. Roles are fleet-fixed — `plan`, `code`, `review`, `verify`, `ops` (`project-board-roles.ts`) — and self-declared per claim, exactly like `task` in `git_join`; `ops` has no lane of its own because it is the role that acts on the open ones. A lane's `allowed_roles`, `wip_limit` and `title` are reshaped with `POST /admin/projects/{slug}/board/columns/{id}`; there is no column create or delete, because deleting one would have to answer what happens to its cards.
...
**A card has a date and an order.** `due_at` is an RFC3339 instant on the card; `depends_on` is a set of card numbers this one waits on, stored as edges in `coord_project_card_deps`. A card whose dependencies have not all reached a terminal lane renders `ready: false` with `waiting_on`, and finishing them clears it with no second edit. Both were previously prose: on the live DNS project, card 5's detail read *"abhängig von Resolver/HA-, Netzwerk- und Puppet-Vorbereitung"* — a dependency on three other cards that nothing could act on and nothing noticed being satisfied. Ordering is advisory like the rest of this board: claiming a card that is not ready succeeds and returns a `depends_unmet` advisory naming what is open. A dependency cycle is the exception and is refused outright, with the chain named (`#5 → #4 → #5`), because there is no reading of it that is the operator's call to make. Archiving a card drops its edges in both directions, so nothing is left waiting on work that can never finish.
...
- `POST /admin/projects/{slug}/board/cards` — create; `POST .../cards/{id}` — edit title/detail/labels/priority/`blocked_reason`/`due_at`/`depends_on` (the console's card editor is a sheet with all of them); `POST .../cards/{id}/move` — `{ column, note? }`; `POST .../cards/{id}/release` — force-release a claim from the console with an optional reason, the escape hatch for a holder that is unreachable but not detectably dead (a wedged process, a sleeping laptop), which neither reclaim signal catches; `DELETE .../cards/{id}`.
