...
11. **Post-exec** — reconcile the native content generation: upload changed
    ...
    one 15-second budget across retries; each request may use the remaining
    time so a successful runner probe is not cancelled after five seconds. It keeps
...
the standard cron spools. Strictly validated spool filenames stay eligible
when static Go `os/user` cannot resolve an NSS/SSSD-only owner;
config-home-owner/sudo/current/root safeguards still require lookup validation.
Reconciliation snapshots each crontab, removes only lines ending in an exact
cxx/cdx/clx managed marker, and preserves all unrelated bytes. Any partial
user cleanup or later legacy-system cleanup failure restores every snapshot
and removes the new shared system entry. Privileged removal uses the same
all-user transaction and restores the snapshots if any system cron file cannot
be removed.
