...
**Git Director** (both capabilities — only when the Git Director service is wired)
- `git_register`, `git_list`, `git_join`, `git_merge_request`, `git_merge_status`, `git_release` — a registry of which agent is working in which clone, plus an advisory arbiter over merges into shared branches. `git_register` announces a worktree, `git_list` shows who else is in the same clone (host-scoped by default), `git_join` declares a task and the paths it expects to write, and `git_merge_request` asks permission to merge, answering `allow`, `wait` or `deny` with the reason and the overlapping files. Gated at runtime by the `git_director_enabled` flag in `versions`, and additionally by managed MCP actually reaching the host — unlike Agent Messaging, these tools ride the orchestrator's own MCP entry rather than a wrapper-local server. The arbitration unit is the clone on a host (`git rev-parse --git-common-dir`), so all linked worktrees of one checkout contend as one; clones sharing a normalized remote are grouped across hosts for visibility but never arbitrated together. The orchestrator has no filesystem access, so every git fact is reported by the caller and the verdict is advisory — nothing prevents a merge, and no hooks are installed. `git_merge_request` is idempotent on a caller-generated `client_request_id`, because a retried call would otherwise enqueue a phantom second contender. A lease is the granted request row itself and expires on a TTL; `git_merge_status` renews it or re-decides a `wait` in place under the same `request_id`, and promotion is poll-driven rather than pushed. Agents that vanish mid-task are the normal case, so reclaiming is first-class: a registration whose bound agent address has no live session is dropped at once, one that goes quiet expires on its TTL, and either releases the lease with it so a branch is never held by something that no longer exists. Any call naming a worktree refreshes it — `git_list` included, since polling for peers is what a working agent actually does — so a busy agent is not reaped for being silent, and reclaimed rows stay visible in the `stale` list instead of vanishing. An operator can also release a registration directly from the console for the case neither signal covers — one still technically alive that a human already knows is finished. Contended decisions may be judged by a model, and fall back to a deterministic `wait` whenever none is reachable, so an inference outage never blocks a merge.

**File transfer** (both capabilities — only when the transfer service is wired)
- `transfer_list`, `transfer_put`, `transfer_get`, `transfer_info`, `transfer_delete` — a fleet-wide, TTL'd pool of arbitrary files, so agents can hand each other a build artifact, a heap dump or a tarball rather than only text. `transfer_put` uploads base64 bytes and returns an id, `transfer_get` fetches them back with a `content_sha256` to verify against, `transfer_info` reads metadata without moving bytes, and `transfer_delete` retires one early. Both put and get take an `offset`, so a file larger than one JSON-RPC message moves in chunks — put with `final: false` then seal with `final: true`, get while `truncated` is true using the previous `next_offset`. Gated at runtime by the `transfers_module_enabled` flag in `versions`, and additionally by managed MCP actually reaching the host, like the Git Director; while it is off `transfer_list` returns disabled status/capabilities with an empty array rather than failing, so an available-but-empty pool is distinguishable from a switched-off one. `ttl_seconds` is required on upload and there is no never sentinel: the service clamps the request into the operator's band and reports the `expires_at` actually granted, and a sweep — on read and, uniquely in this codebase, also on a timer, because what it reclaims is disk — unlinks the bytes when the deadline passes. Bytes live on the `DATA_ROOT` volume rather than in a column, which is why an operator can stream a download from the console. The pool has no addressing: any agent that knows an id can fetch it, nobody is notified of an upload, and handing the id over is the uploader's job — so every fetch is recorded in `agent_transfer_events`, which is what the absence of access control owes an operator. Uploader identity is caller-asserted, like `git_register`'s, since every agent on a box shares one host API key. Per-file and fleet-wide byte caps are operator settings and a full pool is refused with a message naming the cap.
...
Both halves of that Claude shape are held back for a host whose own reported wrapper version predates the plugin migration (`0.8.11`). Config syncs ahead of a host's self-update, and an older `clx` still passes the server on the command line while asking Claude Code to register `server:cxx-agent` as a channel. Claude Code resolves a `server:` channel only against the enterprise, managed, user, project and local MCP scopes — never the command line — so on such a host the user-scope entry is the one thing keeping the channel alive, and its tools are still named `mcp__cxx-agent__<tool>`. Below the threshold the orchestrator therefore keeps serving the old entry and the old allowlist; at or above it, the plugin shape. A host that has never reported a version is treated as pre-plugin.
