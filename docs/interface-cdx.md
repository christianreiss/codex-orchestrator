...
1. `flock` on `$XDG_RUNTIME_DIR/cdx.lock` (or `/tmp/cdx-<uid>.lock`) to enforce single-instance per host. If held, the wrapper enters read-only mode and surfaces "Concurrent" on the boot screen with text picked from the auth-decision state. The `cdx` lock is independent from `clx.lock`; Codex and Claude sessions must not force each other into read-only mode.
