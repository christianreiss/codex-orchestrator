...
- On launch it:
  ...
  - Honors the server-side ChatGPT quota policy (`quota_hard_fail` plus `quota_limit_percent`); admins can flip to warn-only or lower the kill threshold in the dashboard (or set `CODEX_QUOTA_HARD_FAIL=0` / `CODEX_QUOTA_LIMIT_PERCENT=<percent>` before running `cdx`). Hosts marked VIP always receive `quota_hard_fail=false`, regardless of the global policy, so the wrapper warns but never aborts on quota.
