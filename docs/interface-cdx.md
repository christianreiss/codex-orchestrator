...
- Upstream `codex` CLI not found for `run`/`resume`/`exec`/`execute`/`profile` → cxx 0.9.3+ asks `/cron/check` for the target version and installs it in the foreground behind a progress line (skipped when the server answers `disable`); only if that fails is the `cdx cron run` repair hint printed; exit 127.
