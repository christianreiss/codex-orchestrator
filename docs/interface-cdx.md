...
- `cdx` is downloaded per host via `/wrapper/download`; the script is baked with `BASE_URL`, `API_KEY`, `FQDN`, optional CA file, and the host security flag (`secure`, defaults to on). Insecure hosts start with a 30-minute provisioning window so initial sync works immediately; dashboard “Enable” re-opens a 10-minute sliding window (each `/auth` call extends it) and “Disable” closes it immediately (no store grace).
...
- On launch it:
  - Pulls `auth.json` from `/auth` (store/retrieve). If the API is unreachable it proceeds with cached auth when `last_refresh` is ≤24h old (or ≤7 days for secure hosts, with a warning). It refuses to start when the API key is invalid, the insecure window is closed, the API kill-switch is on, or no recent auth is available.
