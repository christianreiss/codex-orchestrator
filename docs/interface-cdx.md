...
| AGENTS | `POST /agents/retrieve` | None | `~/.codex/AGENTS.md` (when canonical storage is empty, the server bootstraps it once from the checked-in repo `AGENTS.md`; served content may append managed Skills and Memories inventory blocks when MCP is enabled and host/runtime data exists, with missing memory summaries lazily backfilled on retrieval when the runner is available) |
