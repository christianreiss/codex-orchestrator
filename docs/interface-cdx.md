...
- When Codex auto-update checks are skipped, the `Versions` section reports the concrete reason: `active cdx run`, `cron-managed auto-update enabled`, `unsupported platform (...)`, or, for genuine privilege skips, `need root; uid ...` with the wrapper-detected UID to make namespace/user mismatches visible.
