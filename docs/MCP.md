...
- Host-authenticated `/mcp` advertises the host-safe memory, resource, Skill, and project tools. Coordinator filesystem helpers (`fs_*`) are not listed and return “method not found” if called on that route.
...
- Fleet secrets store: `secret_list`, `secret_search`, `secret_get`, `secret_store`, `secret_delete`.
  - The working credentials agents need to do their job — GitHub tokens, database passwords, Bookstack/Checkmk tokens, SSH keys, third-party service keys — as opposed to the engine-boot auth that gets an agent started, which lives in `auth_payloads` and never surfaces here. Host capability, gated by the `secrets_module_enabled` flag: while it is off `secret_list`/`secret_search` return nothing and `secret_get` answers `secrets_disabled`. Values are never written to a host filesystem, so revoking one takes effect on the next call. `secret_get` writes its own `mcp_access_logs` row carrying the slug, before returning the value and without swallowing a failure, so an unaudited read cannot happen. Reading is open to every host; **writing is not**. `secret_store` and `secret_delete` only accept a slug this host created — a secret an operator created through the admin API (`source_host_id` NULL) or one another host owns is refused with `secret_not_owned`, so a shared infrastructure credential cannot be repointed by whichever agent guesses its slug. `secret_list` marks each row `owned_by_you` so an agent can tell without asking.
