...
- Fleet secrets store: `secret_list`, `secret_search`, `secret_get`.
  - The working credentials agents need to do their job — GitHub tokens, database passwords, Bookstack/Checkmk tokens, SSH keys, third-party service keys — as opposed to the engine-boot auth that gets an agent started, which lives in `auth_payloads` and never surfaces here. Read-only over MCP, host capability, and gated by the `secrets_module_enabled` flag: while it is off `secret_list`/`secret_search` return nothing and `secret_get` answers `secrets_disabled`. Values are never written to a host filesystem, so revoking one takes effect on the next call. `secret_get` writes its own `mcp_access_logs` row carrying the slug, before returning the value and without swallowing a failure, so an unaudited read cannot happen.
