...
- `resources/templates/list` exposes templates `memory` (`memory://{key}`), `project` (`project://{slug}`), `project_file` (`project://{slug}/files/{stored_name}`), `project_memory` (`project://{slug}/memory/{key}`), `skill` (`skill://{slug}`), and — only when the shared-memory service is wired — `shared_memory` (`shared://{slug}`).
