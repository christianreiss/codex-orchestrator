...
- **Insecure hosts are temporary**: Hosts marked `secure=false` get a sliding 10-minute window (`insecure_enabled_until`) for API use; admin can extend/disable. Outside that window, requests are blocked (403 `insecure_api_disabled`). Use only behind a trusted network during bootstrap.
- **Installer tokens**: Single-use UUID tokens (`install_tokens` table) with TTL (`INSTALL_TOKEN_TTL_SECONDS`, default 1800s). Creating a new token deletes any prior pending token for that host. Tokens are stored in plaintext—treat DB access as sensitive.
