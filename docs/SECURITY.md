...
- **Password reset**: Recovery emails are only sent when `ADMIN_PASSWORD_RESET_FROM` is configured; reset tokens expire (`ADMIN_PASSWORD_RESET_TTL_SECONDS`, clamped between 300 and 86400 seconds). Protect mail infrastructure and reset links.
- **API key binding**: Host API keys are IP-bound on first successful `/auth` or wrapper fetch; later calls from a different IP are 403 unless roaming is allowed or an admin toggles the roaming flag. Reverse DNS enforcement (when enabled) requires forward A/AAAA + PTR match. Runner-only bypass can be enabled via `AUTH_RUNNER_IP_BYPASS=1` + `AUTH_RUNNER_BYPASS_SUBNETS` CIDRs.
...
- **Insecure hosts are temporary**: Hosts marked `secure=false` get a configurable sliding window (`insecure_enabled_until`) for API use (0–480 minutes, default 10, set via the log-ish admin slider/`duration_minutes`). Admins can extend/disable; each `/auth` call refreshes the window by the configured duration. When the window expires naturally, a trailing grace period (`INSECURE_GRACE_MINUTES`, default 60; store-only) permits final auth/usage pushes. Explicitly disabling the window clears the grace period and blocks both `retrieve` and `store` immediately with 403 `insecure_api_disabled`. Optional domain auto-allow rules can auto-open windows for matching subdomains while active. Use only behind a trusted network during bootstrap.
