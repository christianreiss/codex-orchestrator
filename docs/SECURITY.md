...
- **Global rate limit**: Configured via `RATE_LIMIT_GLOBAL_PER_MINUTE` and `RATE_LIMIT_GLOBAL_WINDOW` (defaults 120 req/60s). It runs on every non-OPTIONS request, including `/admin/*`; only `/healthz`, `/admin/ws`, `/admin/_app/`, `/admin/manual/articles/` and `/admin/favicon` are skipped.
