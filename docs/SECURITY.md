...
- **Admin sessions**: Admin login uses an HTTP-only session cookie (`ADMIN_SESSION_COOKIE`) with a configurable TTL (`ADMIN_SESSION_TTL_SECONDS`). Treat admin cookies as sensitive and ensure TLS is enforced end-to-end.
- **Password reset**: Recovery emails are only sent when `ADMIN_PASSWORD_RESET_FROM` is configured; reset tokens expire (`ADMIN_PASSWORD_RESET_TTL_SECONDS`). Protect mail infrastructure and reset links.
...
- **Insecure hosts are temporary**: Hosts marked `secure=false` get a configurable sliding window (`insecure_enabled_until`) for API use (0–480 minutes, default 10, set via the log-ish admin slider/`duration_minutes`). Admins can extend/disable; each `/auth` call refreshes the window by the configured duration. Outside that window, requests are blocked (403 `insecure_api_disabled`). Optional domain auto-allow rules can auto-open windows for matching subdomains while active. Use only behind a trusted network during bootstrap.
