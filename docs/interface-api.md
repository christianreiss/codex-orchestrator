...
- `POST /sync/status` — batched startup diff probe for AGENTS.md and baked `config.toml`. Requires host API key + IP binding. Body accepts local digests (`agents.sha256`, `config.sha256`) plus optional `config.username`/`config.home` and optional `host_user` (`username`/`hostname`) for host-user tracking. Response returns `status` (`ok|update`), deduplicated `reasons`, per-component status blocks (`agents`, `config`), and `host_users`. The `agents` block includes the canonical `base_sha256`, optional `managed_sha256`, and stable `sections` diagnostics for `skills`, `memories`/`memory_routing`, `projects`, and `browseros`. Each section reports whether its concise runtime hint is present, why it is absent when applicable, and its digest when rendered; Skills may also report the available count and engine transport. The metadata describes hints, never a per-Skill or per-memory inventory. Skills are not a standalone cdx startup file channel: Codex reads `skill://{slug}` / `skill://{slug}/{path}` through MCP, while Claude receives native directory bundles in the bootstrap response described below. Optional `include_auth` (defaults true unless explicitly `false`) adds an `auth` block using `/auth` retrieve semantics (including `chatgpt`); for insecure hosts, insecure-window enforcement applies when this auth block runs. Contract: `docs/contracts/sync-status.schema.json`.
...
- `POST /agents/retrieve` — pull the served agent document (`AGENTS.md` for Codex, `CLAUDE.md` for Claude). Optional body `sha256` (64-hex) lets the server return `status:unchanged` without echoing the file. Response includes `status` (`updated` | `unchanged` | `missing`), `version_id`, final `sha256`, canonical `base_sha256`, optional `managed_sha256`, `updated_at`, final `size_bytes`, `sections`, and `content` when updated. Canonical history stores base Markdown only. At render time the server removes any prior orchestrator-owned feature block and appends at most one block delimited by `<!-- cxx:managed-features:start -->` and `<!-- cxx:managed-features:end -->`; its fixed order is Skills, Memory, Projects, then BrowserOS. The block contains concise usage/discovery guidance only: it never enumerates Skills, memories, or projects. Skills guidance makes the fleet MCP authoritative for Codex, requires `skill_list` before any local/system `SKILL.md`, and routes creation, mutation, deletion, and workflow questions to `skill://skill-manager`; Claude is pointed at native `~/.claude/skills/<slug>/SKILL.md`. Memory guidance appears only with effective orchestrator MCP and routes fleet, project, and host scopes through `shared_memory_*`, `project_memory_*`, and `memory_*`. It treats recorded decisions, conventions, runbooks, and handoffs as authoritative over assumptions while requiring mutable code/runtime facts to be verified against the present repository or system and stale records to be updated or deleted instead of duplicated. Projects guidance appears only when Projects and MCP are enabled and names `#coco` / `project_*`; BrowserOS guidance appears only for Codex hosts with both BrowserOS and MCP enabled. `sections.skills`, `sections.memories`/`sections.memory_routing`, `sections.projects`, and `sections.browseros` expose stable presence/reason/digest diagnostics for those hints. The served `sha256` covers base plus the rendered managed block, while `base_sha256` remains the immutable canonical digest; feature-state changes therefore trigger `updated` without altering canonical history. When `status=missing`, the wrapper preserves any existing local document and writes nothing. Per-host overrides (`agents_document_id_override`) take precedence over the fleet setting. When canonical storage is empty, boot requests seed it once from the checked-in repo `AGENTS.md`; later admin-edited canonical versions are not overwritten by that bootstrap file.
...
- MCP memories (host API key auth, or short-lived insecure-host MCP bearer auth, same rate limits) remain host-scoped and are not valid for CoCo cross-server handoffs — use the project memories above for coordination state tied to a workstream, or the shared memories below for fleet-wide reference documents:
...
- Shared memories (host API key auth, same rate limits and IP rules) are the fleet-wide corpus: one row per `slug`, readable and writable from every host and either engine, and scoped to neither host nor project. `source_host_id`/`source_engine` record who wrote last but never filter reads. Documents run to 1 MiB and are chunked for retrieval, so listings and searches return previews and passages rather than bodies. No `^coco` key reservation applies here:
...
- `GET /admin/theme` / `POST /admin/theme` — read/set the stored admin UI theme (`auto` default). The selected value also rides along in the `/auth` `versions` block as `admin_theme`.
...
- The global bucket runs on every non-OPTIONS request, `/admin/*` and `/go/api/*` included; the only bypasses are `/healthz`, `/admin/ws`, `/admin/_app/`, `/admin/manual/articles/`, `/admin/favicon` and immutable `/go/assets/`. Requests with no resolvable client IP are counted against `0.0.0.0`. Tune the env vars above to tighten the windows; they must be positive integers, so the guard cannot be switched off that way.
