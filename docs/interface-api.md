...
- `POST /agents/retrieve` — pull the served agent document (`AGENTS.md` for Codex, `CLAUDE.md` for Claude). Optional `sha256` supports unchanged responses. Canonical history stores base Markdown only; render time replaces one marker-managed block in fixed Skills, Memory, Projects, BrowserOS, Secrets, API keys in chat order. Secrets guidance appears whenever both its module and managed MCP are enabled, including count zero, names the full five-tool lifecycle, and requires `secret_list` before availability or write-capability claims. The independent, cross-engine API-keys-in-chat policy appears whenever its fleet flag is enabled, even without MCP. Response diagnostics include `sections.secrets` and `sections.api_keys_in_chat` alongside the existing feature sections; the served digest covers base plus managed content while `base_sha256` remains canonical. Missing canonical content preserves an existing local document, and per-host document overrides still take precedence.
...
- `GET /admin/doctor` — admin-only read-only Doctor page snapshot: `{generated_at, rows[], hosts, canonical_auth}`. Wraps `SetupStatusService.status()`'s checks and adds rows for the encryption keyring (active key id, configured key count, a synchronous encrypt/decrypt canary), the master/OpenAI/Claude kill switches (a disabled switch is `warn`, not `fail` — it is an intentional admin action), and issued-key active/total counts per engine. Every row is `{id, label, status: "ok"|"warn"|"fail", detail, hint, owner_route}`; `owner_route` is a link only, never an inline control.
- `POST /admin/doctor/test-key` — admin-only; body `{engine:"openai"|"claude", key}`. Fires a minimal models-list request through the same auth/kill-switch/rate-limit path `/v1/models` and `/anthropic/v1/models` use, keyed only on the pasted `key` — it never looks up or decrypts a stored key by id. Returns `{ok, status, latency_ms, model_count?, error?}`. A passing test hits the same per-key, per-caller-IP rate-limit bucket a normal call would and increments `use_count` — it is not a free/invisible probe.
