...
- `POST /agents/retrieve` — pull the served agent document (`AGENTS.md` for Codex, `CLAUDE.md` for Claude). Optional `sha256` supports unchanged responses. Canonical history stores base Markdown only; render time replaces one marker-managed block in fixed Skills, Memory, Projects, BrowserOS, Secrets order. Secrets guidance appears whenever both its module and managed MCP are enabled, including count zero, names the full five-tool lifecycle, and requires `secret_list` before availability or write-capability claims. Response diagnostics include `sections.secrets` alongside the existing feature sections; the served digest covers base plus managed content while `base_sha256` remains canonical. Missing canonical content preserves an existing local document, and per-host document overrides still take precedence.
