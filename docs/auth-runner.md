...
- Runner request payload always includes `auth_json`, `base_url`, and `timeout_seconds`; when host metadata is available it also includes `api_key` and `fqdn`.
...
- Code defaults: `AUTH_RUNNER_IP_BYPASS=0` and `AUTH_RUNNER_BYPASS_SUBNETS=''`. Docker Compose defaults override these to `1` and `172.28.0.0/16,172.30.0.0/16`.
...
- `RUNNER_DEBUG_DUMP_AUTH` (runner container): when `1`, writes last probe payload to `/tmp/last-auth.json` (mode `0600`).
