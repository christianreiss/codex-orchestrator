...
- **versions** — key/value version store (`name`, `version`, `updated_at`) used for the cached client version from GitHub, canonical payload pointer, runner metadata (including `runner_last_check`, `runner_last_fail`, and the daily preflight marker `daily_preflight`), and flags such as `api_disabled` and `quota_hard_fail`.
