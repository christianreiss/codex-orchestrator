...
- **hosts** — `id`, `fqdn`, `api_key` (sha256 hash), `api_key_hash` (same hash for legacy lookups), `api_key_enc` (secretbox-encrypted plaintext API key for baking wrappers), `status`, `secure` (default `1`), `allow_roaming_ips`, `vip` (default `0`, bypasses quota hard-fail), **`insecure_enabled_until`**, **`insecure_grace_until`**, **`insecure_window_minutes`** (desired sliding window length), `last_refresh`, `auth_digest`, `ip`, `client_version`, `wrapper_version`, `api_calls`, `created_at`, `updated_at`.
...
- **client_config_documents** — canonical `config.toml` (`id` always 1, `sha256`, `body`, `settings` JSON, optional `source_host_id` FK, `created_at`, `updated_at`; indexed by `updated_at`).
- **mcp_memories** — MCP memory store scoped per host (`id`, `host_id` FK, `memory_key` VARCHAR(128) unique per host, `content` LONGTEXT, `metadata` JSON nullable, `tags` JSON array nullable, `tags_text` TEXT for full-text search, `created_at`, `updated_at`, `deleted_at`). Full-text index over `content` + `tags_text`; cascades delete with host.
...
- **install_tokens** — single-use installer tokens (`token` SHA-256 hash, `token_enc` ciphertext, `host_id` FK, `fqdn`, `api_key` SHA-256 hash, `api_key_enc` ciphertext, `base_url`, `expires_at`, `used_at`, `created_at`); creation wipes prior pending tokens for the host so only one is active.
- **versions** — key/value version store (`name`, `version`, `updated_at`) used for the cached client version from GitHub (`client_available`), canonical payload pointer (`canonical_payload_id`), runner metadata (`runner_state`, `runner_last_ok`, `runner_last_fail`, `runner_last_check`, `runner_boot_id`, `daily_preflight` last preflight ISO timestamp), and flags such as `api_disabled`, `quota_hard_fail`, and `quota_limit_percent`.
