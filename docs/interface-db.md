...
- **versions** — key/value version store (`name`, `version`, `updated_at`) used for the cached client version from GitHub, canonical payload pointer, runner metadata (including `runner_state`, `runner_last_ok`, `runner_last_fail`, `runner_last_check`, `runner_boot_id`, and the daily preflight marker `daily_preflight`), and flags such as `api_disabled` and `quota_hard_fail`.
