...
- Builder defaults include `notice.model_migrations` entries for retired models (`gpt-5.1-codex-max`, `gpt-5.1-codex-mini`, `gpt-5.2-codex`, `gpt-5.2`, `gpt-5.3-codex`, `gpt-5.4`, `gpt-5.4-mini`) to `gpt-6-astra` so Codex upgrade prompts can be auto-resolved from fleet-managed config. This is the intended state, not necessarily the stored one — `notice.model_migrations` is operator-entered template data, not code-derived, so it drifts silently when the default model changes and nobody revisits it. **Found drifted 2026-08-08**: the live template mapped 3 of the then-retired IDs to `gpt-5.4` instead of the then-current default, was missing `gpt-5.2` and `gpt-5.3-codex`, and additionally carried a wrong `gpt-5.3-codex-spark` → `gpt-5.4` entry. Fix stored template drift with a one-off admin-API write; there is no code-level template default to repair. **Still drifted 2026-09-06**: the live `~/.codex/config.toml` on a managed host continues to map `gpt-5.2-codex` / `gpt-5.1-codex-max` / `gpt-5.1-codex-mini` / `gpt-5.3-codex-spark` to `gpt-5.4`, which is itself a retired id now — and `gpt-5.3-codex-spark` is a currently supported model that must not be migrated away at all. `gpt-5.4-mini` joined the retired set on 2026-08-31 and should map to `gpt-5.6-luna` (its catalog-named replacement), not to Astra.
...
- Dropped feature keys: `steer`, `collaboration_modes`, `elevated_windows_sandbox`, `experimental_windows_sandbox`, `enable_experimental_windows_sandbox`, `remote_models`, `request_permissions`, `request_rule`, `responses_websockets`, `responses_websockets_v2`, `search_tool`, `sqlite`, `use_linux_sandbox_bwrap`, `web_search_cached`, `web_search_request`, `js_repl`, `tui_app_server`, `voice_transcription`, `apply_patch_freeform`, `apps_mcp_path_override`, `code_mode_buffered_exec`, `codex_git_commit`, `enable_fanout`, `external_migration`, `image_detail_original`, `item_ids`, `js_repl_tools_only`, `local_thread_store_shared_compression`, `multi_agent_mode`, `plugin_hooks`, `remote_control`, `resize_all_images`, `send_async_message`, `skill_env_var_dependency_prompt`, `terminal_resize_reflow`, `tool_search`, `tool_search_always_defer_mcp_tools`, `unavailable_dummy_tools`, `undo`, `unified_exec_zsh_fork`, `workspace_owner_usage_nudge`.
...
- `apply_patch_freeform`, `apps_mcp_path_override`, `code_mode_buffered_exec`, `codex_git_commit`, `enable_fanout`, `external_migration`, `image_detail_original`, `item_ids`, `js_repl_tools_only`, `local_thread_store_shared_compression`, `multi_agent_mode`, `plugin_hooks`, `remote_control`, `resize_all_images`, `send_async_message`, `skill_env_var_dependency_prompt`, `terminal_resize_reflow`, `tool_search`, `tool_search_always_defer_mcp_tools`, `unavailable_dummy_tools`, `undo`, `unified_exec_zsh_fork`, `workspace_owner_usage_nudge` moved here 2026-09-06: `codex features list` on codex-cli 0.153.4 reports all of them at stage `removed`, so toggling any is a no-op. Only `removed` keys were taken — stage `deprecated` flags (`use_legacy_landlock`, and the `web_search_*` pair already listed above) still do something. Note `request_permissions` in this list has never matched an upstream flag; the real one is `request_permissions_tool`, which is `under development` and deliberately NOT dropped. The filter is exact-match, so the stale entry is inert rather than harmful.
