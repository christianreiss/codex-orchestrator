...
- `GET /wrapper/v2/config` — signed per-host wrapper config JSON (`{payload, signature}`); `?sig=1` returns the detached signature as `text/plain`. `signature` is always the primary (oldest active) key's, so several active signing keys stay invisible to deployed wrappers; `x-signature-fingerprint` names the key that produced it.
