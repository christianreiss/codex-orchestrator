...
### Agents
- `POST /agents/retrieve` — retrieve the served engine document (`AGENTS.md` or `CLAUDE.md`). Optional `sha256` enables `status:unchanged` without content. Returns `status` (`updated` | `unchanged` | `missing`), `version_id`, final `sha256`, canonical `base_sha256`, optional `managed_sha256`, `sections`, `updated_at`, final `size_bytes`, and `content` when updated. Canonical history contains only the admin-authored base. The served copy may append one `cxx:managed-features` block with concise engine/host-specific hints for Skills, MCP Memory, Projects/CoCo, Codex-only BrowserOS, Secrets, and the cross-engine API-keys-in-chat policy; it never embeds a per-Skill, per-secret, per-memory, or per-project inventory. Codex Skill guidance declares the fleet MCP authoritative, requires `skill_list` before local/system Skill files, and routes management/workflow requests to `skill://skill-manager`. Feature-state changes affect the final/managed digests without changing `base_sha256`.
...
- Agents: `GET /admin/agents`, `GET /admin/agents/render?host_id={id}&engine=codex|claude`, `GET /admin/agents/versions/{id}`, `POST /admin/agents/store`, `POST /admin/agents/serve`, `POST /admin/agents/revert`, `POST /admin/agents/retention`, `DELETE /admin/agents/versions/{id}`. The render endpoint previews the selected host's exact effective document, including dynamic managed guidance, without recording a host sync. `revert` takes `{version_id, engine?}` and republishes that version; `retention` sets `{backup_limit}` (null clears the stored limit).
