...
- MCP resources include host-scoped memories (`memory://{id}`), canonical Skill manifests (`skill://{slug}`), and, when the Projects module is enabled, shared project bootstrap resources (`project://{slug}`). Clients should prefer `skill://{slug}` as the primary Skill read path; synced local `SKILL.md` files remain fallback copies, and shared CoCo state still belongs only in project resources.
