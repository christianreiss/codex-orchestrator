...
- `POST /admin/hosts/{id}/model` — set per-host Codex `model_override` / `reasoning_effort_override` and Claude `claude_model_override` (null/empty clears). Codex supports `gpt-6-astra`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.5`, `gpt-5.4-mini`, and `gpt-5.3-codex-spark`; effort must be valid for the selected model. Astra is the fleet default at `medium` and supports `low|medium|high|xhigh|max`; Sol/Terra additionally support `ultra`, Luna stops at `max`, and GPT-5.5/GPT-5.4 mini/Spark support `low|medium|high|xhigh`. Stored retired Codex overrides are backfilled to Astra with the intentionally retained `high` migration effort.
