...
- Syncs `~/.codex/config.toml`, `~/.codex/AGENTS.md`, slash command prompts, and Skills (`~/.agents/skills/`) via `/sync/status` + `/sync/bootstrap` (with fallback to per-surface endpoints). Managed Skills such as the project-module `coco` skill stay read-only and are not pushed back to the server from the client; remote MCP clients can also read them through `skill://{slug}` resources (for example `skill://coco`).
