...
- Enable Joplin from Settings → Joplin only after the saved Joplin Server URL plus account email/password pass the built-in connection test. This module now talks only to Joplin Server sessions and sync items, not the local Web Clipper/Data API. The first successful enable still runs an immediate full import of all reachable notes plus the current notebook list before the module is considered live, and `Sync now` re-runs that same full import on demand.
