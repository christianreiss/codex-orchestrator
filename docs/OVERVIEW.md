...
- Extras ride the same API: Skill distribution, native project coordination (notes/todos/files/feedback/activity), host-scoped MCP memories, the fleet-wide shared memory corpus (`shared_memory_*`), ChatGPT `/wham/usage` snapshots.
...
- Logs, Skills, project coordination tables, shared memory documents and their chunk index, ChatGPT snapshots, and version flags all live in MySQL; storage is the compose volume.
