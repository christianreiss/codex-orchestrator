...
4) **Wrapper distribution**
   ...
   - Plain SSH terminals use an interactive compatibility bridge: when `python3` is available, `cdx` strips Codex kitty keyboard enable/disable sequences and normalizes CSI-u Enter/Ctrl keys before launch so prompts submit normally over SSH; `cdx doctor` reports SSH env hints plus the bridge state.
