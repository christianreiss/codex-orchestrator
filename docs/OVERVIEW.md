...
5) **Host telemetry**
   ...
   - Host sync uses `/skills` list/retrieve/store. Agents can also create/update/revive shared manifest-only Skills with MCP `skill_store` and write recoverable delete markers with `skill_delete`; the managed `skill-manager` Skill documents that flow. When project coordination is enabled, the same delivery path also ships managed `coco`.
