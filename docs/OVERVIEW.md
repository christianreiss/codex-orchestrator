...
1) **Provision a host (admin)**
   ...
   - `GET /install/{token}` emits a bash script that downloads the baked wrapper, installs Codex from GitHub (Linux + macOS `apple-darwin` assets), applies the SSH safeguard fallback for blocklisted Codex builds (`0.113.0 -> 0.112.0`) when the installer runs over SSH, prints versions plus a compact usage quickstart (`cdx --version`, `cdx`, `cdx --execute ...`), and leaves `cdx` ready to run. Tokens expire (`INSTALL_TOKEN_TTL_SECONDS`) and are marked used on first fetch.
...
4) **Wrapper distribution**
   ...
   - Plain SSH terminals get an explicit Codex CLI safeguard: interactive SSH runs treat Codex `0.113.0` as blocked and target `0.112.0` instead; `cdx doctor` reports SSH env hints plus the current safeguard state.
...
- Build/edit `config.toml` from `/admin/config.html`; saved output is synced by `cdx` to `~/.codex/config.toml` baked per host (managed HTTP MCP entry; secure hosts use the host API key, insecure hosts get a short-lived bearer). Default rendered features include `multi_agent = true`; `status:missing` deletes the local copy. Legacy feature keys (`steer`, `experimental_windows_sandbox`, `enable_experimental_windows_sandbox`) remain ingest-compatible but are dropped from rendered output.
