...
- Enable Joplin from Settings → Joplin only after the saved clipper/Data-API URL plus access token pass the built-in connection test. The panel can now request access directly from Joplin through the official approval handshake (`POST /auth` then polling `/auth/check`) instead of requiring a manual token paste every time. The first successful enable still runs an immediate full import of all reachable notes plus the current notebook list before the module is considered live, and `Sync now` re-runs that same full import on demand.
