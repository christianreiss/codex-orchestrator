...
- **`api/src/services/agents.ts` + `host-agents.ts`** — store versioned canonical base editions and serve fleet or per-host selections. At host render time, one marker-managed block adds applicable Skills, MCP Memory, Projects, Codex-only BrowserOS, Secrets, and the optional cross-engine API-keys-in-chat policy. Secrets guidance remains present for an enabled empty store so `secret_store` can create its first row; canonical base history remains unchanged.
