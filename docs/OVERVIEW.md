...
- **`public/index.php` router** — boots env, migrations, key manager + secretbox, encryption migrator, repositories/services, scheduled preflight (8h), global rate limiting, and all routes (host/admin/installer/slash/skills/agents/config/MCP/usage/pricing/chatgpt).
...
- **`AgentsService`** — stores versioned AGENTS.md editions, serves either the latest or a pinned version, and feeds the admin editor + host sync.
...
- Admin dashboard supports login + role-based access once at least one active admin user exists; userless installs behave as before until the first admin is created. Admin users, roles, and password recovery live in the Users panel.
...
1) **Provision a host (admin)**
   ...
   - `GET /install/{token}` emits a bash script that downloads the baked wrapper, installs Codex from GitHub (latest tag the API knows about), prints versions, and leaves `cdx` ready to run. Tokens expire (`INSTALL_TOKEN_TTL_SECONDS`) and are marked used on first fetch.
