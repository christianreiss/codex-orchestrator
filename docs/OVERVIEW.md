...
- Enable shared project coordination from Settings → Projects when you want multi-agent notes/todos/files/feedback; that toggle also auto-publishes the managed `coco` skill to `~/.agents/skills/coco/SKILL.md` on syncing clients. The Settings panel stays compact and opens each project on its own `#project-detail/<slug>` workspace page.
