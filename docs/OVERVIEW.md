...
- Enable shared project coordination from Settings → Projects when you want multi-agent notes/todos/files/feedback; that toggle also auto-publishes the managed `coco` skill with embedded toolkit/help to `~/.agents/skills/coco/SKILL.md` on syncing clients, and the same manifest is readable remotely through MCP `skill://coco` when clients cannot rely on local skill files. Disabling the module removes that managed skill on the next sync. Shared CoCo handoffs are project-only; host-scoped MCP memories are not a cross-server fallback. The Settings panel stays compact and opens each project on its own `#project-detail/<slug>` workspace page.
