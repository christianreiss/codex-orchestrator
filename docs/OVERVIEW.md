...
- Post-run output now ends with a compact footer (`Run usage`, `Run cost`, `Sync`) that includes server-calculated run cost on a dedicated `💰` line when `/usage` returns `data.cost`; displayed cost is rounded to two decimals with a trailing `$` (example `0.43$`).
