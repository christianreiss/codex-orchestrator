...
- Enable Joplin from Settings → Joplin only after the saved URL/token pass the built-in connection test. The first successful enable now runs an immediate full import of all reachable notes plus the current notebook list before the module is considered live, and `Sync now` re-runs that same full import on demand.
