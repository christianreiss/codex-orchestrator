...
Small Node 22 + Fastify + Drizzle + MySQL service that keeps canonical Codex and Claude credentials for every host in your fleet. Hosts talk to `/auth` (retrieve/store) with per-host API keys baked into their `cdx`/`clx` wrappers. The same API also ships Skills, shared project coordination, fleet memories and secrets, agent-to-agent messaging, a TTL'd file transfer pool, token-usage telemetry, and ChatGPT/Claude usage snapshots.
...
- Canonical auth + per-target tokens are encrypted with libsodium `secretbox`; the key is generated into `.env` by `bin/install.sh` (never at runtime — the API refuses to boot without one). Optional keyring mode (`ENCRYPTION_KEYS` + `ENCRYPTION_ACTIVE_KID`, or the legacy `AUTH_ENCRYPTION_KEYS` + `AUTH_ENCRYPTION_ACTIVE_KID`) supports rotation with `kid`-tagged ciphertext.
- Safety rails: API kill switch, token quality checks, RFC3339 timestamp bounds, optional IP roaming, and opt-in insecure-host gates.
...
- Extras ride the same API: canonical Skill distribution (including an optional,
  ...
  (`shared_memory_*`), the fleet secrets store, encrypted Agent Messaging
  across Codex and Claude, the Git Director merge arbiter, the project Kanban
  board, a TTL'd file transfer pool (`transfer_*`), and ChatGPT `/wham/usage`
  snapshots.

## Key components (code map)

- **`api/src/server.ts` boot** — boots env, key manager + secretbox, Drizzle client, services, and registers all routes under `api/src/routes/*` (host/admin/installer/seed/auth/sync/skills/projects/agents/config/MCP/chatgpt/versions). It also starts every in-process background worker under `api/src/ops/`: `auth-verification-worker.ts` (canonical credential re-probing), `auth-retention-worker.ts` (daily prune of superseded auth generations after 180 days), `mattpocock-skills-worker.ts`, `agent-portal-worker.ts`, `agent-messaging-worker.ts`, `insecure-fleet-window-worker.ts` (closes the fleet-wide insecure window on its deadline, 30-second tick) and `agent-transfers-worker.ts` (expired transfer bytes off disk every `TRANSFERS_PURGE_INTERVAL_SECONDS`). Boot checks (`boot-checks.ts`) and pending migrations (`boot-migrations.ts`, `RUN_MIGRATIONS_ON_BOOT` default on) run before the listener opens. Drizzle mirrors the schema (`api/src/db/schema.ts`), but it cannot express FULLTEXT indexes or foreign keys, so the hand-written SQL under `api/src/db/migrations/` is the DDL source of truth — see `docs/interface-db.md` for why `drizzle:push` is not the mechanism.
...
- **`api/src/services/project-board.ts` + `project-board-roles.ts` + `api/src/routes/admin/project-board/`** — cards as the unit of coordinated work on a project, with claim/lease semantics mirroring the Git Director. There is no lease table and no card-history table: the live claim IS the `coord_project_cards` row, and every create, move, claim, release and reclaim is a `coord_project_events` row with `entity_type='card'`, so board activity reaches the `project_changes` poll agents already run. Roles are a fixed fleet vocabulary (`plan`, `code`, `review`, `verify`, `ops`) declared per claim rather than stored against an agent, so the board does not depend on Agent Messaging being on. Claims expire after 30 minutes, are renewed implicitly by any call naming the card, and are reclaimed on two signals: a bound `agent_bus_addresses` row with no live session, immediately, and silence past the TTL as the fallback. Advisory throughout — a role that does not match a lane and an exceeded WIP limit attach an advisory to a move that still happens, and the only refusal is recording a claim somebody else holds. Migration 0026 moved every `coord_project_todos` row onto a card **and kept its id as the card number**, so `project_todo_*` is now a view of the same rows with the same ids; that table is retained but no longer written. MCP surface is `project_board_list` (the no-argument discovery entry point, which answers `status: "disabled"` rather than failing when the module is off) plus `project_card_create|claim|move|release|update|get`, gated by `project_board_enabled`.
- **`api/src/services/host-sync.ts`** — computes combined startup diffs/payloads for the engine agent document and config (`/sync/status`, `/sync/bootstrap`) so wrappers can reduce pre-run API fan-out. The agent portion uses the effective served document, so host/engine-specific managed feature guidance and fleet policies such as API keys in chat participate in startup diffing; sync payloads expose `base_sha256`, `managed_sha256`, and stable per-feature presence/reason/digest diagnostics without copying Skill, secret, or memory inventories.
- **`api/src/services/agents.ts` + `agent-policy-composer.ts` + `host-agents.ts`** — store immutable canonical base editions and their optional structured builder provenance, then serve fleet or per-host selections. Every effective document receives a non-removable managed prefix identifying Codex Orchestrator fleet membership, instruction precedence/safety floor, and explicit Hard Stop Lines. The canonical middle is composed from toggleable operating modules plus custom instructions; the existing managed suffix adds applicable Skills, MCP Memory, Projects, Codex-only BrowserOS, Secrets, and the optional cross-engine API-keys-in-chat policy. Fresh databases receive the complete default composition, while only the exact known version-55 digest is automatically converted into a new rollback-safe builder version.
- **`api/src/services/shared-memories.ts` + `shared-memory-chunker.ts`** — the fleet-wide shared memory corpus: slug-addressed documents up to 1 MiB, chunked on markdown structure and FULLTEXT-indexed per chunk, with `shared_memory_list` (no query — the discovery entry point), `shared_memory_search` (ranked passages, `degraded: true` when the index is missing), `shared_memory_read` (bounded windows with `next_offset`), `shared_memory_write` (sha-guarded whole-body replace), `shared_memory_append` (multi-writer safe), and `shared://{slug}` resources. Managed guidance requires an offset-zero complete read with one stable digest before replacement so a partial window cannot amputate the document. Scoped to neither host nor project — `source_host_id`/`source_engine` are provenance only, never read filters.
- **`api/src/services/secrets.ts`** — the fleet secrets store for working credentials, separate from engine-boot auth. Values remain encrypted and only admin reveal or MCP `secret_get` decrypts. MCP exposes the full `secret_list` / `secret_search` / `secret_get` / `secret_store` / `secret_delete` lifecycle, with explicit live capability metadata and host ownership gates for mutations. The store does not itself write plaintext to hosts, plaintext reads are audited before return, and managed guidance permits task-requested persistence or relay while preferring tool-native parameters, stdin, inherited descriptors, or process-scoped environment variables.
- **`wrappers/cxx/internal/remote/` + `api/src/services/remote-exec.ts`** — the default-off `cxx remote` family (`remote_exec_enabled`), a process API on machines the fleet reaches over SSH. There is no daemon on either side: ssh(1)'s own `ControlMaster` is the reused connection, so authentication happens once per destination and every later command is a channel on it. Job state is a directory on the target rather than memory in a process, which is what makes a cursor a byte offset into a real file, an idempotency key an exclusive `mkdir`, a reconnect a read rather than a re-run, and a job started before a reboot reportable as lost instead of running. The target needs only SSH access — no engine, no API key, no signed config — and the binary installs itself there on first contact. The switch reaches wrappers as `remote.enabled` in the signed host config (`wrappers/schemas/host-config-v1.json`) and agents as a section in served AGENTS.md/CLAUDE.md (`api/src/services/managed-agents-features.ts`). It is a fleet control and not a boundary: an agent that can run `cxx remote` can already run `ssh`, and `docs/SECURITY.md` says so plainly rather than letting the switch read as confinement.
...
- **`api/src/services/agent-transfers.ts` + `api/src/ops/agent-transfers-worker.ts` + `api/src/routes/admin/transfers/`** — the default-off, fleet-wide file transfer pool (`transfers_module_enabled`). Unlike every other artifact store the content is not a column: bytes land under `DATA_ROOT/transfers/<shard>/<id>` and `agent_transfers` holds metadata plus an append-only `agent_transfer_events` audit trail. MCP surface is `transfer_list` (answers `status: "disabled"` rather than failing), `transfer_put` (base64 chunks of up to 4 MiB with `offset`/`final` for larger files; `ttl_seconds` required and clamped into the operator band, the reply reporting the deadline actually granted), `transfer_get`, `transfer_info` and `transfer_delete`. There is no addressing — any agent with the id can fetch — so every fetch is recorded instead; reading a file back in the console needs `transfers.download` (owner/admin), separate from `transfers.manage`. Expiry is swept on read and by the worker, which unlinks bytes before flipping the row so a crash leaves a retry rather than an orphan.
...
1) **Provision a host (admin)**
   ...
   - `GET /install/{token}` emits a POSIX shell script that fetches every enabled signed config first, refuses a dual-engine install unless wrapper version/SHA metadata is identical, downloads one `cxx`, atomically replaces legacy regular wrappers with relative enabled aliases, prepares Node.js/npm best-effort when Claude is requested (without them the Claude CLI installs from the registry's native platform package), and bootstraps each matching CLI. The compact setup view ends in `READY` only when all requested components verify; partial installs end in `INCOMPLETE` with a non-zero exit and direct retry commands. Tokens expire after a TTL fixed at 1800s in the API and are marked used on first fetch.

2) **Every `/auth` call**
   - The auth-verification worker runs on boot and then every `AUTH_RUNNER_VERIFY_WORKER_INTERVAL_SECONDS` (default 300s), re-probing Codex/Claude canonical payloads on a dynamic schedule from `AUTH_RUNNER_VERIFY_TTL_SECONDS` (default 900s, minimum) up to `AUTH_RUNNER_VERIFY_MAX_INTERVAL_SECONDS` (default 21600s); successful gateway traffic counts as verification and keeps probes idle. Successful stale probes also refresh per-engine runner telemetry, so the runner card reflects auth readiness for `cdx`/`clx` startup. `/auth retrieve` reads that stored verdict and does not run runner probes inline.
...
- Versions: reports the effective fleet Codex target (GitHub latest with stale fallback plus an internal minimum floor of `0.125.0`), `client_version_enforce_exact` downgrade policy, common wrapper version/sha from server disk, runner state, quota policy (`quota_hard_fail`, `quota_limit_percent`, and optional `quota_week_partition` pacing), `auto_update_enabled` for managed update hosts, and the fleet-wide `cdx_silent` quiet flag. From cxx 0.8.2, auto-update runs in shared background maintenance: a host-staggered 15-minute cron plus a detached launch-triggered catch-up. Foreground startup and exit never download, install, reconcile peer binaries, or re-exec for an upgrade. Native releases are staged separately and validated before an atomic cache-pointer activation; existing sessions keep their installed files. A per-user coordinator lease prevents overlap, failures retry after five minutes, and `cxx cron run` forces a check. Auth and content sync remain active when binary auto-update is off. When Codex self-management is skipped, the summary note still distinguishes active-run, unsupported-platform, and true privilege-skip cases; privilege skips still include the wrapper-detected UID to expose root/user-namespace mismatches directly in the output. VIP hosts force warn-only (`quota_hard_fail=false`) regardless of the global policy.
...
- **Traffic shaping** — No in-process request-rate limiter is installed. Deployments that need volumetric controls provide them at the trusted reverse proxy or network edge.
...
- File transfers always expire: the row's `expires_at` is mandatory, bytes are unlinked from `DATA_ROOT/transfers/` on read and by `api/src/ops/agent-transfers-worker.ts` (every `TRANSFERS_PURGE_INTERVAL_SECONDS`, default 300, with an orphan-file reap every twelfth tick), and the metadata row plus its `agent_transfer_events` audit trail are kept.
...
- Bring up the stack with `bin/install.sh`, which generates `.env`, provisions the schema (`migrate.js --init-schema` applies the baseline only against an empty database, then migrates), builds the wrapper fleet and creates the first owner. Twelve re-runnable steps; `READY` is printed only when all six critical `/readyz` checks pass, and `bin/install.sh doctor` names the command that fixes each failure. Add `--tls acme|file|selfsigned` for the bundled Caddy frontend. Runner + quota cron sidecars are on by default in compose.
- Then open `/admin/setup` (linked from the installer's closing message; starts at Engines when the installer already created the owner). The nine-step first-run wizard covers infrastructure, the owner claim, engines, credentials, fleet defaults, agent policy, modules, collaboration, and an optional first host. Only the first two block; progress persists server-side, so an interrupted run resumes from the dashboard card. Its Fleet defaults step is the write that creates the `client_config_documents` row — without it the managed feature context reports `config_missing` and skills, memory, projects and secrets stay disabled regardless of their own switches.
- Log into Codex once on a trusted box; seed that `~/.codex/auth.json` from the wizard's Credentials step or **Hosts → More → Seed canonical auth**, use the one-time `curl | bash` seed command, or call `/auth` with `command: "store"`.
