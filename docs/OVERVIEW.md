...
- **`RunnerVerifier`** — HTTP client to the auth-runner; probes readiness, posts canonical auth, requests skill summaries, requests admin skill drafts, and returns runner telemetry.
...
- **`StartupSyncService`** — computes combined startup diffs/payloads for AGENTS.md and config (`/sync/status`, `/sync/bootstrap`) so wrappers can reduce pre-run API fan-out. The AGENTS portion uses the effective served document, so managed runtime additions like the Skills inventory block also participate in startup diffing.
