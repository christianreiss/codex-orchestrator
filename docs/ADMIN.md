...
## Access & Auth
- Base path: `/admin/`.
- mTLS is enforced when `ADMIN_REQUIRE_MTLS=1` (default). If you disable it, gate the path another way (VPN/firewall) and optionally set `DASHBOARD_ADMIN_KEY` for a shared secret overlay.
...
- **Hosts**:
  ...
  - Actions per host: enable/disable insecure window (2–60 min slider; each `/auth` extends it), toggle secure vs insecure (insecure hosts purge `~/.codex/auth.json` after each run), toggle roaming IPs, toggle IPv4-only (re-bakes curl -4 and clears pinned IP), mark VIP (quota never hard-fails), clear canonical auth (reset digest/last_refresh), delete host, view canonical auth (`include_body`); re-register (New Host) to mint a fresh installer token.
...
- **Reopen insecure window**: Hosts → select host → set duration (2–60) → enable. Each `/auth` call extends by that duration.
