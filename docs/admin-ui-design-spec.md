...
- **Trust through transparency**: surface state, costs, and quotas without burying them.
...
**Purpose:** At-a-glance fleet health, cost, and quota state. The most-visited page.
...
4. **Charts grid** (`#dashboardGrid`) — generated by JS, contains one shell with two charts:
   ...
   - **Cost trend** chart (`#dashboardCostCanvas`) — token spend over time, grouped by component.
...
**Table columns:** Time · Host · Client IP · Input · Output · Cached · Reasoning · Cost — all
...
2. **Usage** — Pricing reference table (Model / Input/1K / Output/1K / Cached/1K).
...
- Cost trend canvas: `#dashboardCostCanvas`.
...
- **Reduce gradient usage.** Current `--surface-tint`, `--table-header`, `--accent-gradient`
  ...
  Keep gradients only for: the accent CTA button (subtle), and the dashboard chart's cost area
  fill (functional).
...
- **Numbers in mono with tabular-nums**: every cost, count, percent in tables and dashboards
