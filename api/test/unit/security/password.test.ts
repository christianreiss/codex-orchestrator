...
describe('password.verify', () => {
  ...
  it('verifies a $P$ phpass hash and proposes an argon2 rehash', async () => {
    // Generated by Openwall's phpass PortableHash under PHP 8.4, not by our port.
    const stored = '$P$BwpZDaX7tB76m7FRB4IfCwImvrI2sj0';
    const result = await verify(stored, 'hunter2');
    expect(result.ok).toBe(true);
    expect(result.rehash?.startsWith('$argon2id$')).toBe(true);
  });

  it('rejects a wrong password for a $P$ phpass hash without rehashing', async () => {
    const stored = '$P$BwpZDaX7tB76m7FRB4IfCwImvrI2sj0';
    const result = await verify(stored, 'hunter3');
    expect(result.ok).toBe(false);
    expect(result.rehash).toBeUndefined();
  });
