...
- shared_memory_* — fleet-wide reference documents that outlive any single project: runbooks, architecture notes, accumulated findings. Scoped to neither host nor project, up to 1 MiB each, chunked and searchable. Start with shared_memory_list (it needs no query), narrow with shared_memory_search, read with shared_memory_read. Append for concurrency, replace for correction: shared_memory_append adds new material without a read-modify-write race, but when an existing statement has become WRONG, rewrite that slug with shared_memory_write (passing expected_sha256) or drop it with shared_memory_delete. Appending beside a stale fact leaves both standing, and the next agent cannot tell which one is true.
...
export function managedCocoBootstrapGuidance(): {
  ...
  return {
    ...
    quickstart: [
      ...
      'If anything you read turns out to be wrong, correct it before you finish: shared_memory_write on the same slug, or shared_memory_delete when it is superseded. Do not leave the correction sitting next to the stale record.',
    ],
  };
}
