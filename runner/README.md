...
- `RUNNER_CODEX_PROBE_MODEL` (environment) — model used for the Codex “Reply Banana” probe; defaults to `gpt-6-astra`.
...
- Runs `/usr/local/bin/codex exec --model "${RUNNER_CODEX_PROBE_MODEL:-gpt-6-astra}" -s read-only --skip-git-repo-check "Reply Banana if this works."`.
